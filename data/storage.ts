...
// ⚠️ QUATTRO FORMATI, DUE STRADE DIVERSE.
//
// JPEG e PNG sono immagini ferme: si ritagliano 3:1 e si riducono a
// 1200x400 come si e' sempre fatto, e il PNG resta PNG per non perdere
// la trasparenza — un logo su fondo trasparente ri-codificato in JPEG
// si porta dietro un rettangolo nero.
//
// GIF e WebP possono essere ANIMATE, e li' il ritaglio non si puo'
// fare: un canvas legge un fotogramma solo, quindi ritagliare vorrebbe
// dire consegnare allo sponsor la sua animazione ridotta a una figurina
// ferma. Quei due si caricano COSI' COME SONO, e il taglio 3:1 lo fa la
// fascia — visivamente, senza toccare il file.
// ⚠️ DUE MEGABYTE E NON QUATTRO: i riquadri sono diventati dieci, e
// la fascia nell'app precarica i banner per averli pronti alla
// rotazione. Con quattro megabyte l'uno, il socio che apre la Home di
...
export async function caricaSponsorSfide(circoloId: string, file: File, indice: number): Promise<string> {
  ...
  if (formato.animabile) {
    if (file.size > PESO_MASSIMO_ANIMATA) {
      const mb = (file.size / (1024 * 1024)).toFixed(1);
      throw new Error(
        `Questa ${formato.estensione.toUpperCase()} pesa ${mb} MB: il massimo è ${PESO_MASSIMO_ANIMATA / (1024 * 1024)} MB. `
        + 'Un banner troppo pesante si carica lentamente sul telefono dei soci con poca linea.',
      );
    }
    return await salvaBannerSponsor(circoloId, indice, file, formato);
  }
  const img = await caricaImmagine(file);
  ...
  const blob = await new Promise<Blob>((resolve, reject) => {
    canvas.toBlob(
      ...
      formato.tipo,
      // Sul PNG la qualita' non esiste — e' senza perdita, e il
      // parametro viene ignorato: si passa 1 per non far credere il
      // contrario a chi legge.
      formato.estensione === 'png' ? 1 : 0.85
    );
  });
  return await salvaBannerSponsor(circoloId, indice, blob, formato);
}

// Il pezzo comune alle due strade: scrive il file, aggiorna l'elenco
// del circolo e toglie di mezzo quello che ha sostituito. Non sa da
// dove viene il blob — ritagliato o originale — e non deve saperlo.
async function salvaBannerSponsor(
  circoloId: string, indice: number, blob: Blob, formato: FormatoBanner,
): Promise<string> {
  ...
  // ⚠️ L'estensione segue il formato vero: un file .jpg che dentro e'
  // una GIF confonde le cache e i browser.
  const riferimento = ref(
    storage,
    `sponsor_sfide/${circoloId}/sponsor_${Date.now()}.${formato.estensione}`,
  );
  // ⚠️ Il tipo si DICHIARA. Le regole dello Storage adesso accettano
  // solo image/*, e il blob che esce da fetch() su un file locale
  // arriva spesso senza tipo o come application/octet-stream: senza
