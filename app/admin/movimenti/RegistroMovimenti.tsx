...
// ============================================================
// REGISTRO MOVIMENTI — il corpo della schermata, senza la porta.
//
// Pensato per il caso concreto della contestazione: si filtra per
// socio e periodo, si legge la catena dei saldi, e se serve si stampa
// il prospetto da consegnare a mano. La stampa usa il dialogo del
// browser con un foglio di stile dedicato: nessuna libreria, e chi
// vuole un file lo salva in PDF da li'.
//
// ⚠️ E' UN COMPONENTE E NON PIU' UNA PAGINA, dal 29 agosto 2026. Lo
// stesso registro serve a due persone diverse che entrano da due porte
// diverse: l'Admin del circolo, che vede il proprio, e il team Racket
// Fever, che deve poter guardare quello di un circolo qualunque quando
// arriva una contestazione. Le due porte hanno controlli d'accesso
// diversi — un responsabile o un collaboratore da una parte, un Super
// Admin dall'altra — ma quello che c'e' dentro e' identico.
//
// ⚠️ COPIARLO SAREBBE STATO PIU' VELOCE E SBAGLIATO. Questo progetto ha
// gia' pagato due volte il prezzo di due copie che divergono: i gemelli
// delle traduzioni separati per tre tornate senza che nessuno se ne
// accorgesse. Un registro contabile che mostra numeri diversi a seconda
// di chi lo apre e' molto peggio.
//
// Chi entra passa `circoloId` e dove tornare; il resto e' tutto qui.
// ============================================================
...
export default function RegistroMovimenti({
  circoloId, tornaHref, tornaEtichetta = '← Dashboard', sottotitolo,
}: {
  circoloId: string;
  tornaHref: string;
  tornaEtichetta?: string;
  // Il team ha bisogno di sapere DI CHI e' il registro che sta
  // guardando; l'Admin no, perche' ne ha uno solo.
  sottotitolo?: string;
}) {
  ...
  useEffect(() => {
    if (!circoloId) return;
    const u1 = ascoltaCircolo(circoloId, setCircolo);
    const u2 = ascoltaMovimentiCircolo(circoloId, setMovimenti);
    return () => { u1(); u2(); };
  }, [circoloId]);
  ...
  const filtrati = useMemo(() => {
    ...
    return movimenti.filter((m) => {
      // ============================================================
      // ⚠️ «LEZIONI» NON E' UN TIPO, ed e' il motivo per cui quel
      // pulsante non ha mai filtrato niente: cadeva nel confronto qui
      // sotto, `m.tipo !== 'lezioni'` era sempre vero, e l'elenco
      // restava vuoto. Una lezione e' un ADDEBITO con un maestro
      // scritto sopra — lo dice anche il commento accanto alla
      // costante — quindi si riconosce da li'.
      // ============================================================
      if (tipo === FILTRO_LEZIONI) {
        if (!m.maestroNome) return false;
      } else if (tipo !== 'tutti' && m.tipo !== tipo) return false;
      // ⚠️ E il filtro per maestro non veniva applicato affatto: era
      // nelle dipendenze del calcolo — quindi il ricalcolo scattava —
      // ma dentro non lo leggeva nessuno. Due pulsanti che si
      // accendevano senza fare niente.
      if (maestroFiltro && m.maestroNome !== maestroFiltro) return false;
  ...
  return (
    <div className="admin-root">
      <div className="admin-header no-print">
        <button className="admin-btn-small" onClick={() => router.push(tornaHref)}>
          {tornaEtichetta}
        </button>
        <h1 className="admin-header-title">Registro Movimenti</h1>
        <p className="admin-header-sub">
          {sottotitolo ?? 'Ogni ricarica, addebito e rimborso del circolo, in ordine di data.'}
        </p>
      </div>
